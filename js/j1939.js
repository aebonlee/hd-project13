...
(function (root, factory) {
  ...
  function parseId(v) {
    ...
    // ⚠ ID 는 기획서 정의상 "16진수 7~8자리" 표기다. 숫자만 있어도(글자가 안 섞여도)
    //   자릿수가 7~8이면 그 자체가 16진 표기다 — 접두어를 빼먹은 사람의 실수를 그대로 받아야 한다.
    //   예) '18000401' → 0x18000401 (10진 18000401 로 읽으면 다른 메시지가 되어 버린다)
    if (/^\d{7,8}$/.test(s)) return parseInt(s, 16) >>> 0;
    // 그 밖의 자릿수(숫자만)는 관행상 10진으로 넘어온 값으로 본다 (호출부에서 숫자 그대로 줄 수도 있음)
